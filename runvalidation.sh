...
while true; do
  CUDA_VISIBLE_DEVICES=1 python -u eval.py --dataset_dir=/mnt/data/Rohit/ACMData/1a_CATVideosTrain/tfval/ --train_log_dir=logs/train/ --number_of_steps=1 --num_batches=3832 --split=validation --eval_log_dir=logs/val/cat/ |& tee -a log_1acat
  sleep 20
  ...
  CUDA_VISIBLE_DEVICES=1 python -u eval.py --dataset_dir=/mnt/data/Rohit/ACMData/5aSynthVideosE2E/tfval/  --train_log_dir=logs/train/ --number_of_steps=1 --num_batches=312 --split=test --eval_log_dir=logs/val/synth/ |& tee -a log_5asynth
  sleep 20
done
